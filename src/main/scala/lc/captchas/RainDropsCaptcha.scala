...
class RainDropsCP extends ChallengeProvider {
  ...
  def returnChallenge(): Challenge = {
    ...
    val drops = dropsOrig ++ extendDrops(dropsOrig, 1, xOffset) ++ extendDrops(
      dropsOrig,
      2,
      xOffset
    ) ++ extendDrops(dropsOrig, 3, xOffset)
    ...
    for (i <- 0 until 60) {
      ...
      val textWidth = g
        .getFontMetrics()
        .charsWidth(secret.toCharArray, 0, secret.toCharArray.length)
